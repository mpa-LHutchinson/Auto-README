...
if (process.argv.includes('--mc') || process.argv.includes('--mc')) {
  ...
  if (outputIndex < process.argv.length) {
    ...
    if (!Number.isInteger(modelNumber)) {
      console.error(
        'Error: Did not specify model number. Make sure to specify an integer number after --mc'
      );
      process.exit(1);
    ...
    for (let i = 0; i < models.length; i++) {
      ...
      createREADME(fileContents, fileNames, i)
        .then(async (readmeContent) => {
          console.log(
            'Successfully generated README with model ' +
              models[i] +
              '. Check model' +
              i +
              '.md in this directory for your new README file.'
          );
          fs.writeFileSync(
            path.join(__dirname, `Model${i}.md`),
            readmeContent,
            'utf8'
          );

          if (
            process.argv.includes('--token-usage') ||
            ...
            console.error(
              `Token usage for ${models[i]}: ${tokenUsage.prompt} tokens in the prompt, ${tokenUsage.completion} tokens in the completion.`
            );
          }
        })
        .catch((error) => {
      ...
      .then(async (readmeContent) => {
        console.log(
          'Successfully generated README with model ' +
            ...
            'check ' +
            readmeFileName +
            ' in this directory for your new README file.'
        );
        ...
        if (
          ...
          console.error(
            `Token usage: ${tokenUsage.prompt} tokens in the prompt, ${tokenUsage.completion} tokens in the completion.`
          );
        }
      })
      .catch((error) => {
